...
diff_samples = random_samples_post(:,1) - random_samples_pre(:,1);
D_DIP = mean(diff_samples);
BCI_DIP(:,1) = prctile(diff_samples, [2.5, 97.5]);
d=(mean(random_samples_post) - mean(random_samples_pre))/sqrt((var(random_samples_post)+var(random_samples_pre))/2);
...
hold on;
lower_error = filtered_diff(sorted_idx) - filtered_CI(1, sorted_idx)';
upper_error = filtered_CI(2, sorted_idx)' - filtered_diff(sorted_idx);
errorbar(1:length(sorted_idx), filtered_diff(sorted_idx), lower_error, upper_error, 'k', 'LineWidth', 1.5, 'LineStyle', 'none');
ylabel('Condition 2 - Condition 1');
xtickangle(90);
title(paramsvec{ii});
ax = gca;
ax.XGrid = 'on';
